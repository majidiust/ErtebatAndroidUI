...
import org.ertebat.ui.dialogs.MessageDialog;
import org.ertebat.ui.dialogs.WaitingDialog;
...
public class BaseActivity extends FragmentActivity implements ITransport {
	...
	public static final String PICTURES_DIRECTORY_NAME = "Ertebaat";
	...
	protected IWebsocketServiceCallback mWebsocketServiceCallback = new IWebsocketServiceCallback.Stub() {
		...
		@Override
		public void loggedInResult(int code, String details) throws RemoteException {
			Log.d(TAG, String.valueOf(code) + " : " + details);
		}

		@Override
		public void newMessage(String type, String messageId, String fromId, String fromUsername, String roomId,
				String date, String time, String content) throws RemoteException {
			if (mTransportCallback != null) {
				mTransportCallback.onNewMessage(new MessageSchema(type, messageId, fromId, fromUsername, roomId, date,
						time, content));
			}
		}
		...
		@Override
		public void currentProfileResult(String username, String userId, String firstName, String lastName,
				String mobile, String email) throws RemoteException {
			if (mTransportCallback != null) {
		...
		@Override
		public void friendAdded(String userName, String id, String status) throws RemoteException {
			try {
		...
		@Override
		public void membersAddedToRoom(String roomId, String memberId) throws RemoteException {
			if (mTransportCallback != null) {
		...
		@Override
		public void notifyAddedByFriend(String invitedBy) throws RemoteException {
			if (mTransportCallback != null) {
		...
		@Override
		public void notifyAddedToRoom(String invitedBy, String roomId) throws RemoteException {
			if (mTransportCallback != null) {
		...
		public void onFriendConnectivityStatusChanged(String friendId,
				String connectivityStatus) throws RemoteException {
			if (mTransportCallback != null) {
	...
	protected ServiceConnection mWebsocketServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(ComponentName name, IBinder service) {
			mWebsocketService = IWebsocketService.Stub.asInterface(service);
			try {
	...
	// TODO: @Majid, load the contacts into this list and use it anywhere you
	// need. I have used it for adding a contact to a chat
	...
	protected static boolean sIsShowingDialog = false;
			...
			mSipBroadCastRecv = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					// Begin Of SIP Invite Segment
						...
						case TERMINATED:
							if (NgnMediaType.isAudioVideoType(mediaType)) {
								// Show UI Function that indicate to terminated
								// call
								// NgnInviteEventTypes
						...
						case INCOMING:
							if (NgnMediaType.isAudioVideoType(mediaType)) {
								if (mAVSession != null) {
									...
									// CHECK
									// Intent incomingIntent = new Intent(This,
									// CallActivity.class);
									// incomingIntent.putExtra(NgnEventArgs.EXTRA_EMBEDDED,
									// args);
					...
					// Chat Section
						...
						case INCOMING:
							...
							// ShowIncomingMessage(remoteParty, dateString,
							// event.getContent());
							// ReloadMessageList();
							break;
						default:
					...
					// handled by the owner of the context
						...
						case DATA:
							...
							NgnHistorySMSEvent event = new NgnHistorySMSEvent(NgnUriUtils.getUserName(session
									.getRemotePartyUri()), StatusType.Incoming);
							event.setContent(content == null ? NgnStringUtils.nullValue() : new String(content));
							mEngine.getHistoryService().addEvent(event);
							// ShowIncomingMessage("Pager", "Now",
							// event.getContent());
							// ReloadMessageList();
							break;
						default:
	...
	protected void signInNGN() {
		...
		mConfigurationService.putString(
				NgnConfigurationEntry.IDENTITY_PASSWORD, logStrValue = mSharedPref.getString("pref_key_password", ""));
		Log.d("SignInNGN", logStrValue);

		mConfigurationService.putString(
				NgnConfigurationEntry.NETWORK_PCSCF_HOST, logStrValue = mSharedPref.getString("pref_key_CSCFHost", ""));
		Log.d("SignInNGN", logStrValue);

		logStrValue = mSharedPref.getString("pref_key_port", "0");
		mConfigurationService.putInt(NgnConfigurationEntry.NETWORK_PCSCF_PORT, Integer.parseInt(logStrValue));
		Log.d("SignInNGN", logStrValue);
		...
		mConfigurationService.putString(
				NgnConfigurationEntry.NETWORK_REALM, logStrValue = mSharedPref.getString("pref_key_network_realm", ""));
		Log.d("SignInNGN", "Realm: " + logStrValue);

		mConfigurationService.putBoolean(
				NgnConfigurationEntry.NETWORK_USE_WIFI, logBoolValue = mSharedPref.getBoolean("", true));
		Log.d("SignInNGN", String.valueOf(logBoolValue));

		mConfigurationService.putBoolean(
				NgnConfigurationEntry.NETWORK_USE_3G, logBoolValue = mSharedPref.getBoolean("", true));
		...
		// TODO: Change
		mConfigurationService.putString(
				NgnConfigurationEntry.RCS_CONF_FACT,
				logStrValue = mSharedPref.getString("pref_key_RCS_CONF_FACT", "sip:Conferense@doubango.org"));
		mConfigurationService.putString(
				NgnConfigurationEntry.RCS_SMSC,
				logStrValue = mSharedPref.getString("pref_key_RCS_SMSC", "sip:+331000000000@doubango.org"));
		mConfigurationService.putBoolean(
				NgnConfigurationEntry.RCS_USE_BINARY_SMS, logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(
				NgnConfigurationEntry.RCS_USE_MSRP_SUCCESS, logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(
				NgnConfigurationEntry.RCS_USE_MSRP_FAILURE, logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(
				NgnConfigurationEntry.RCS_USE_OMAFDR, logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(
				NgnConfigurationEntry.RCS_USE_MWI, logBoolValue = mSharedPref.getBoolean("pref_key_", true));
...
//	}
	...
	public void getFriendSummery(final String friendId){
		try {
			new Thread(new Runnable() {
				...
				public void run() {
					HttpClient client = new DefaultHttpClient();
					Log.d(TAG, RestAPIAddress.getSignIn());
					HttpGet get = new HttpGet(RestAPIAddress.getFriendSummery() + "/" + friendId);
					...
					try {
						...
						if (response.getStatusLine().getStatusCode() == 200) {
							...
							String id, cs;
							FriendState fcs = FriendState.FS_Offline;
							try{
								cs = jsonObject.getString("state");
								if(cs.compareTo("Online") == 0){
									fcs = FriendState.FS_Online;
								}
							}
							catch(Exception ex){
								logCatDebug(ex.getMessage());
							...
							if (mTransportCallback != null) {
								mTransportCallback.onFriendConnectivityStatusChanged(id, cs);
							}

						}
						else {
							showAlert("دریافت وضعیت دوست با خطا روبرو گردید");
						}
					} catch (Exception ex) {
						Log.d(TAG, ex.getMessage());
					}
						...
						if (response.getStatusLine().getStatusCode() == 200) {
							...
							if (code.compareTo("9") == 0) {
								...
								for (int j = 0; j < listOfFriends.size(); j++) {
									FriendSchema fs = listOfFriends.get(j);
									try {
										if (SessionStore.mSessionStore != null) {
											SessionStore.mSessionStore.addFriend(fs);
											getFriendSummery(fs.m_friendId);
										}
										if (mTransportCallback != null) {
											mTransportCallback.onNewFriend(fs);
										}
							...
							showAlert("دریافت اطلاعات دوستان با خطا روبرو گردید.");
						}
					} catch (Exception ex) {
						Log.d(TAG, ex.getMessage());
					}
				...
				public void run() {
					try {
						...
						int width = (int) getResources().getDimension(
								org.ertebat.R.dimen.chat_message_item_picture_width);
						...
						ContentBody content = new FileBody(pictureFile);// ,
						// "multipart/form-data");
	...
	protected void showAlertDialog(final String message) {
		...
		mDialog = new MessageDialog(This, 0, org.ertebat.ui.dialogs.Dialog.TIMEOUT_NONE, message);
		mDialog.show();
	 ...
	 * 
	 * @param dialogId
	 *            ID with which the dialog is identified upon timeout
	 * @param message
	 *            The message to display
	 * @param timeout
	 *            Number of milliseconds before the dialog times out
	 */
	protected void showAlertDialog(int dialogId, final String message, long timeout) {
		closeDialog();

		mDialog = new MessageDialog(This, dialogId, timeout, message);
		mDialog.show();
	}

	/**
	 * Displays a waiting dialog with a custom message
	 * 
	 * @param message
	 *            The message to display
	 */
	protected void showWaitingDialog(String message) {
		...
		mDialog = new WaitingDialog(This, 0, WaitingDialog.TIMEOUT_NONE, message);
		mDialog.show();
	...
	/**
	 * Displays a waiting dialog with a custom message that can make a callback
	 * after a given amount of time
	 * 
	 * @param dialogId
	 *            ID of the dialog message. Used when handling the time-out
	 *            event
	 * @param timeout
	 *            Amount of time in milliseconds before the dialog closes and
	 *            fires the time-out event
	 * @param message
	 *            The message to display
	 */
	protected void showWaitingDialog(int dialogId, long timeout, String message) {
		closeDialog();

		mDialog = new WaitingDialog(This, dialogId, timeout, message);
		mDialog.show();
	}
	...
	protected void onServerConnectionChanged(boolean isConnected) {
		View root = findViewById(android.R.id.content);
		root.setBackgroundResource(isConnected ? R.color.main_bg : R.color.main_bg_disconnected);
	}
	...
	private void Notify(String notificationTitle, String notificationMessage) {
		...
		@SuppressWarnings("deprecation")
		Notification notification = new Notification(android.R.drawable.ic_dialog_alert, "New Message",
				System.currentTimeMillis());

		Intent notificationIntent = new Intent(this, MainActivity.class);
		PendingIntent pendingIntent = PendingIntent.getActivity(this, 0, notificationIntent, 0);

		notification.setLatestEventInfo(BaseActivity.this, notificationTitle, notificationMessage, pendingIntent);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case DIALOG_MULTI_CHOICE:
			if (resultCode == Activity.RESULT_OK) {
				...
				onMultiChoiceDialogResult(
						data.getExtras().getString("Title"), data.getExtras().getString("Callback"), selected);
			}
			break;
		case DIALOG_FRAGMENT_MULTI_CHOICE:
			if (resultCode == Activity.RESULT_OK) {
				int selected = data.getExtras().getInt("SelectedIndex");
				mFragmentDialogListener.onMultichoiceDialogResult(data.getExtras().getString("Title"), data.getExtras()
						.getString("Callback"), selected);
			}
			break;
		default:
			break;
	...
	public void onConnectedToServer() {
		SessionStore.mSessionStore.setConnectivityState(true);
	...
	public void onDisconnctedFromServer() {
		SessionStore.mSessionStore.setConnectivityState(false);
		for (int i = 0; i < mTransportListeners.size(); i++) {
		...
		if (SessionStore.mSessionStore.addMessageToRoom(ms)) {
			...
			if (ms.mType.equals("Text")) {
				...
				Thread imageLoader = new Thread(new Runnable() {
					...
					public void run() {
						try {
							// URL url = new URL(SettingSchema.mBaseRestUrl +
							// "uploaded/entities/" + ms.mBody);
							// showToast(SettingSchema.mBaseRestUrl +
							// "uploaded/entities/" + ms.mBody);
							...
							// File pictureFileDirectory = new
							// File(Environment.getExternalStoragePublicDirectory(
							// Environment.DIRECTORY_PICTURES).getPath() +
							// "/entities");
							// pictureFileDirectory.mkdirs();
							// File pictureFile = new File(pictureFileDirectory,
							// ms.mId + ".png");
	...
	public void onCurrentProfileResult(String username, String userId, String firstName, String lastName,
		...
		mCurrentUserProfile.m_lastName = lastName;
		for (int i = 0; i < mTransportListeners.size(); i++) {
			((ITransport) mTransportListeners.get(i)).onCurrentProfileResult(
					username, userId, firstName, lastName, mobile, email);
		}
	}
	...
	public void onRoomAdded(String roomName, String roomId, String roomDesc, String roomLogo, String roomType,
		...
		for (int i = 0; i < mTransportListeners.size(); i++) {
			((ITransport) mTransportListeners.get(i)).onRoomAdded(
					roomName, roomId, roomDesc, roomLogo, roomType, members);
		}
	}
	...
	public void onFriendConnectivityStatusChanged(String friendId,
		...
		FriendState fcs = FriendState.FS_Offline;
		if(connectivityStatus.compareTo("Online") == 0){
			fcs = FriendState.FS_Online;
		}
		if (SessionStore.mSessionStore != null) {
			SessionStore.mSessionStore.setFriendConnectivityStatus(friendId, fcs);
		}
		for (int i = 0; i < mTransportListeners.size(); i++) {
