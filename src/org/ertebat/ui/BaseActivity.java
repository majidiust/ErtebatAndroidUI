...
public class BaseActivity extends FragmentActivity implements ITransport {
	...
	protected void signInNGN() {
		...
		// SMS Settings
		// TODO: Change
		mConfigurationService.putString(NgnConfigurationEntry.RCS_CONF_FACT,
				logStrValue = mSharedPref.getString("pref_key_RCS_CONF_FACT", "sip:Conferense@doubango.org"));
		mConfigurationService.putString(NgnConfigurationEntry.RCS_SMSC,
				logStrValue = mSharedPref.getString("pref_key_RCS_SMSC", "sip:+331000000000@doubango.org"));
		mConfigurationService.putBoolean(NgnConfigurationEntry.RCS_USE_BINARY_SMS,
				logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(NgnConfigurationEntry.RCS_USE_MSRP_SUCCESS,
				logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(NgnConfigurationEntry.RCS_USE_MSRP_FAILURE,
				logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(NgnConfigurationEntry.RCS_USE_OMAFDR,
				logBoolValue = mSharedPref.getBoolean("pref_key_", true));
		mConfigurationService.putBoolean(NgnConfigurationEntry.RCS_USE_MWI,
				logBoolValue = mSharedPref.getBoolean("pref_key_", true));
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case DIALOG_MULTI_CHOICE:
			if (resultCode == Activity.RESULT_OK) {
				...
				onMultiChoiceDialogResult(data.getExtras().getString("Title"), data.getExtras().getString("Callback"),
						selected);
			}
			break;
	...
	public void onCurrentProfileResult(String username, String userId, String firstName, String lastName,
		...
		mCurrentUserProfile.m_lastName = lastName;
		for (int i = 0; i < mTransportListeners.size(); i++) {
			((ITransport) mTransportListeners.get(i)).onCurrentProfileResult(username, userId, firstName, lastName,
					mobile, email);
		}
	}
	...
	public void onRoomAdded(String roomName, String roomId, String roomDesc, String roomLogo, String roomType,
		...
		for (int i = 0; i < mTransportListeners.size(); i++) {
			((ITransport) mTransportListeners.get(i)).onRoomAdded(roomName, roomId, roomDesc, roomLogo, roomType,
					members);
		}
	}
