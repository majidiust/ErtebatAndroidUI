...
public class CallFragment extends BaseFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
			...
			public void onClick(View v) {
				if (mBaseActivity.mSipService.isRegistered()) {
					...
					NgnAVSession AVSession = NgnAVSession.createOutgoingSession(
							mBaseActivity.mSipService.getSipStack(), NgnMediaType.Audio);// .AudioVideo);
		...
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (mBaseActivity.mSipService.isRegistered()) {
					...
					final String validUri = NgnUriUtils.makeValidSipUri(remoteUri);
					NgnAVSession AVSession = NgnAVSession.createOutgoingSession(
							mBaseActivity.mSipService.getSipStack(), NgnMediaType.AudioVideo);
					AVSession.setRemotePartyUri(remoteUri); // HACK
					if (AVSession.makeCall(validUri)) {
						ShowToast("Making a call!");
					} else {
		...
		mBtnSipToggle.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!mIsSipConnected) {
					...
					/**
					 * TODO: @Majid, when successfully connected to the SIP
					 * server change the button background like below
					 */
					...
					/**
					 * TODO: @Majid, when successfully disconnected from the SIP
					 * server change the button background like below
					 */
					if (!BaseActivity.IsTablet)
		...
		mViewRemoteVideoPreview = (FrameLayout) rootView
				.findViewById(R.id.view_call_incall_video_FrameLayout_remote_video);
		mViewLocalVideoPreview = (FrameLayout) rootView
		...
		if (args != null) {
			...
			if (mBaseActivity.mAVSession == null) {
				...
				final NgnContact remoteParty = mBaseActivity.mEngine.getContactService().getContactByUri(
						mBaseActivity.mAVSession.getRemotePartyUri());
				String number;
				if (remoteParty != null) {
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				if (mDialedNumber.length() >= MAX_NUMBERS
						|| (mBaseActivity.mCurrentStatus != CallActivityStatus.CAS_Idle && mBaseActivity.mCurrentStatus != CallActivityStatus.CAS_NumberEntry))
					return;
